...
// If Links shouldnt displayed, remove true and add false
// Attention, if you use, links will get forbidden and the "more" comment links gets hidden
// Default is true
$links = true;
$posts_num = 5;
...
function searchSiteForCatKeyword($content)
{
	...
	//global "finbey_singlecat";
		...
		if((strpos($content, '[category_id=')) != false){
			...
			if((strpos($content, '[category_posts=')) != false){
				$pos1 = (strpos($content, '[category_posts='));
				$pos2 = strpos ($content, ']', $pos1);
				$posts = substr($content, $pos1, $pos2-$pos1);
				$posts = str_replace('[category_posts=', '', $posts);
				$posts = str_replace(']', '', $posts);
				$posts_num = (int)$posts;
			}
			
			$cat_cont = loadCategory($cat_id, $posts_num);
	...
	$content = str_replace('[category_posts='.$posts_num . ']', '', $content);
...
function loadCategory($cat_id, $posts_num)
{
	$args = array(
